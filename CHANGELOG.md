...
- **PR-G Phase 2B — v2 measurement readiness (dry-run validated runbook)** — the Phase 2A harness had never been executed; this PR runs the provisional Tier A/B/C dry run end to end on the checked-in real Ibex 32768² fixture and fixes the four defects the dry run exposed (harness worker argv plumbing, GDS `--layer` selection threading — the 27-layer Ibex file refuses implicit layer choice, Tier C `SimulatorResult` access, and a Tier A measurement bias where the reference pass warmed the indexed row cache making the indexed wall read as pure cache hits — reference and indexed now run on INDEPENDENT source instances and first-touch speedup is measured honestly). Dry-run outcome on CPU-only host, exactly per protocol: Tier A `SUCCESS` with exact indexed/reference parity on the real fixture (first-touch discovery ≈135× vs the pre-G1 full-flat scan, 99.6% of flat scans avoided — non-authoritative provisional evidence, final Tier A reruns from the frozen formal commit); Tier B `NOT_RUN_ENVIRONMENT`; Tier C `UNSUPPORTED` (GPU-resident path) with the CPU SOCS finite witness PASS. New `docs/industrial-v2-measurement.md` operational runbook (host requirements, clean-commit freeze, isolated env, fixture identity, preflight, harness-only measurement, acceptance gates, promotion-only-through-`promote_canonical_family()`, verifier → generated claims → README drift gate, honest-outcome rules incl. "weak speedup stays non-headline" and "slower GPU row is diagnostics"); `.gitignore` for v2 mutable run workspaces (only the verified canonical family enters git in Phase 2C). No canonical v2 family, no README GPU numbers; v1.1 and P-054/B04 untouched.
- **PR-G Phase 2A — Industrial Benchmark v2 protocol / measurement readiness** (`benchmark/industrial_v2.py`, `benchmarks/industrial-v2/run_v2_benchmark.py`, `scripts/{preflight,verify_industrial_v2_artifacts,generate_industrial_v2_claims}.py`) — **protocol authority first, measurement second**: this PR ships the complete v2 protocol surface and deliberately publishes NO canonical v2 results (§1: the honest terminal state is `V2 PROTOCOL READY / FORMAL GPU MEASUREMENT PENDING`). New independent namespace (`OpenLithoHub.industrial-benchmark.v2` + `industrial-run-config.v2`, results root `benchmarks/results/industrial-v2/`, frozen 7-member canonical family, all-or-nothing promotion). Run identity binds ONE sha256 to the measurement commit, harness/core/generator/verifier BYTES, fixture hash and every semantic knob (device/dtype/TF32/determinism/compile/batch/tile/halo/pixel/forward params/Hopkins params/warmup/repeats) — hostile matrix proves each mutation changes identity. GPU environment lock records driver/CUDA/cuDNN/devices/TF32 facts (never just a model name); the harness ships Tier A (indexed vs reference exact-vector window discovery with EXACT parity gate), Tier B (CPU batch=1 vs GPU batch=1 vs GPU batch=N with the deterministic finite-support blur, synchronized CUDA timing via a mandatory `cuda_synchronized` timing method, allocated/reserved peak memory as separate facts, fresh worker process per repeat, median/p10/p90/n) and Tier C (fixed Hopkins config; GPU-resident path recorded `UNSUPPORTED` rather than faked). Canonical promotion is a guarded all-or-nothing operation refusing dirty trees, provisional runs, NOT_RUN_ENVIRONMENT rows, missing GPU memory stats, unsynchronized timing, failed correctness, or a CPU-only host (`FORMAL_PUBLICATION_BLOCKED`). Verifier closes manifest/SHA256SUMS/source/identity/claims with fail-closed unknown-file rejection; claim generator emits README numbers ONLY from a verifier-closed family and the `--check` drift gate fails CI on any `IB2-*` README reference without admitted generated claims (B2-J). New fast CI job `industrial-v2-protocol` (schema/identity/manifest hostile tables + CPU-host preflight refusal + claims drift gate — no measurements in CI). Doc repairs (§2): invalid `--num-gpus all` example replaced with the integer semantics, unsupported "8x less memory" claim removed, legacy dense multi-GPU path vs single-device streaming distinguished. `dummy-identity` is declared the PRODUCT GPU PLUMBING SMOKE subject (cpu+cuda, batch-safe — exact semantics), enabling the §41/§42 smoke gates; README states GPU infrastructure is available while formal artifact-backed GPU measurements are pending v2. No canonical v2 family, no GPU headline, no historical-number reuse; v1.1 and P-054/B04 untouched.
