...
def bracket(eps):
    bad = [r["h_px"] for r in profile[:-1] if r["unrestricted_binary_lower_intensity"] > eps]
    necessary = max(bad) + 1 if bad else 0
    sufficient = next(
        (r["h_px"] for r in profile if r["absolute_tail_upper_intensity"] <= eps),
        None,
    )
    return {
        "epsilon_intensity": eps,
